...
///    VERSION 9_4_1 date du 22 decembre 2017
...
void CommPlotProducer::SetPV(TString rootpv, TString rootdata){
    ...
    puweight=false;
    usePUrew=true;
    cout << " PU in data from " << rootdata << endl;
    cout << " PU in MC from " << rootpv << endl;

    TFile *filepuest = new TFile(rootpv);
    TFile *filepudata;
    if (rootdata!="-1") {
       filepudata = new TFile(rootdata);
    }
    else {
       filepudata = new TFile(rootpv);
    }

    TH1D* npu_mc= (TH1D*) filepuest->Get("nPV_mc_unw");
    TH1D* npu_dat= (TH1D*) filepudata->Get("nPV_data");
...
void CommPlotProducer::SetPVdata(TString rootpv, TString rootdata){
    ...
    TH1D* npu_mc= (TH1D*) filepuest->Get("nPV_data");
    TH1D* npu_dat= (TH1D*) filepudata->Get("nPV_data");
 ...
 */
 ...
 if (setprescale) {
   int cmsrun=0,cmsls=0,totprescval=0;
   std::ifstream *fps = &_fPS;
   int c = 0;
        while( !fps->eof() )
          {
             std::string line;
     ...
     /*
     for (int i=0; i<10; i++)
     {  cout <<   _runVal[i]  <<  "   "  << _lbVal[i]  <<  "   "  << _psVal[i] << endl; }
     */
 }
}
...
*/
    ...
    TH1D* nPU_mc                  = new TH1D("nPU_mc",                "nPU_mc",                200,0,200);
    TH1D* nPV_data                = new TH1D("nPV_data",              "nPV_data",              180,0,180);
    TH1D* nPV_data_unw            = new TH1D("nPV_data_unw",          "nPV_data_unw",              180,0,180);
    TH1D* nPV_mc                  = new TH1D("nPV_mc",                "nPV_mc",                180,0,180);
    ...
    TH1D* dPVz_mc                 = new TH1D("dPVz_mc",                "dPVz_mc",              50,0.,0.1);
    ...
    */
    ...
    AddHisto("jet_phi"    ,"phi of all jets",                50,-1.*pi,pi);
    ...
    AddHisto("jet_eta_sv"	  ,"eta of jets containing a SV",     50,-2.5,2.5);
    ...
    AddHisto("jet_eta_csvv2_l"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_eta_csvv2_m"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_eta_csvv2_t"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_phi_csvv2_l"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_csvv2_m"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_csvv2_t"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_negcsvv2_l"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_negcsvv2_m"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_negcsvv2_t"       ,"phi of jets",              50,-1*pi,pi );

    AddHisto("jet_pt_deepcsv_l"       ,"pT of jets",              PtMax/10,0,PtMax );
    AddHisto("jet_pt_deepcsv_m"       ,"pT of jets",              PtMax/10,0,PtMax );
    AddHisto("jet_pt_deepcsv_t"       ,"pT of jets",              PtMax/10,0,PtMax );
    AddHisto("jet_eta_deepcsv_l"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_eta_deepcsv_m"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_eta_deepcsv_t"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_phi_deepcsv_l"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_deepcsv_m"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_deepcsv_t"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_pt_deepfl_l"       ,"pT of jets",              PtMax/10,0,PtMax );
    AddHisto("jet_pt_deepfl_m"       ,"pT of jets",              PtMax/10,0,PtMax );
    AddHisto("jet_pt_deepfl_t"       ,"pT of jets",              PtMax/10,0,PtMax );
    AddHisto("jet_eta_deepfl_l"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_eta_deepfl_m"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_eta_deepfl_t"       ,"eta of jets",              50,-2.5,2.5 );
    AddHisto("jet_phi_deepfl_l"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_deepfl_m"       ,"phi of jets",              50,-1*pi,pi );
    AddHisto("jet_phi_deepfl_t"       ,"phi of jets",              50,-1*pi,pi );


  
    int nMubins=20;
    if (trigname=="btag") nMubins=50;
    if (produceRecoMuon) {
    ...
    AddHisto2D("track_HPix_vs_eta"     ,"number of hits in the Pixel vs eta",                 10,-0.5, 9.5, 50,  -2.5, 2.5);
    AddHisto2D("track_nHit_vs_phi"     ,"number of hits  vs phi",               35,-0.5, 34.5 ,50, -1.*pi,pi);
    ...
    AddHisto2D("track_multi_vs_pv"  ,      "number of tracks in the jets vs pv",                40,-0.5,39.5 ,50, 0., 50.);
    AddHisto2D("trk_multi_sel_vs_pv"  ,    "number of selected tracks in the jets vs pv",       30,-0.5,29.5, 50, 0., 50.);

    AddHisto2D("track_eta_vs_phi_3pixhit"     ,"eta vs phi",              50, -2.5, 2.5, 50,  -1.*pi,pi);
    AddHisto2D("track_eta_vs_phi_3pixl1y"     ,"eta vs phi",              50, -2.5, 2.5, 50,  -1.*pi,pi);
    AddHisto2D("track_eta_vs_phi_3pixl1n"     ,"eta vs phi",              50, -2.5, 2.5, 50,  -1.*pi,pi);
    AddHisto2D("track_eta_vs_phi_4pixhit"     ,"eta vs phi",              50, -2.5, 2.5, 50,  -1.*pi,pi);

    AddHisto("track_IP_3pixhit"     ,      "3D IP of tracks (3pix)" ,                           nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_3pixhit"   ,     "3D IP error (3pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_3pixhit"    ,      "3D IP significance (3pix)",         nTrackbins,-35.,35.  );
    AddHisto("track_len_3pixhit"     ,     "decay length (3pix)",                               nTrackbins,0,5.     );
    AddHisto("track_dist_3pixhit"    ,     "distance to the jet axis (3pix)",                    nTrackbins,0.,0.08    );
    AddHisto("track_IP_3pixl1y"     ,      "3D IP of tracks (3pix)" ,                           nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_3pixl1y"   ,     "3D IP error (3pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_3pixl1y"    ,      "3D IP significance (3pix)",         nTrackbins,-35.,35.  );
    AddHisto("track_len_3pixl1y"     ,     "decay length (3pix)",                               nTrackbins,0,5.     );
    AddHisto("track_dist_3pixl1y"    ,     "distance to the jet axis (3pix)",                    nTrackbins,0.,0.08    );
    AddHisto("track_IP_3pixl1n"     ,      "3D IP of tracks (3pix)" ,                           nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_3pixl1n"   ,     "3D IP error (3pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_3pixl1n"    ,      "3D IP significance (3pix)",         nTrackbins,-35.,35.  );
    AddHisto("track_len_3pixl1n"     ,     "decay length (3pix)",                               nTrackbins,0,5.     );
    AddHisto("track_dist_3pixl1n"    ,     "distance to the jet axis (3pix)",                    nTrackbins,0.,0.08    );
    AddHisto("track_IP_4pixhit"     ,      "3D IP of tracks (4pix)" ,                           nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_4pixhit"   ,     "3D IP error (4pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_4pixhit"    ,      "3D IP significance (4pix)",         nTrackbins,-35.,35.  );
    AddHisto("track_len_4pixhit"     ,     "decay length (4pix)",                               nTrackbins,0,5.     );
    AddHisto("track_dist_4pixhit"    ,     "distance to the jet axis (4pix)",                    nTrackbins,0.,0.08    );

    AddHisto("track_IP_3pixhit_eta05"     ,      "3D IP of tracks (3pix)" ,                             nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_3pixhit_eta05"   ,     "3D IP error (3pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_3pixhit_eta05"    ,      "3D IP significance (3pix)",           nTrackbins,-35.,35.  );
    AddHisto("track_len_3pixhit_eta05"     ,     "decay length (3pix)",                         nTrackbins,0,5.     );
    AddHisto("track_dist_3pixhit_eta05"    ,     "distance to the jet axis (3pix)",                    nTrackbins,0.,0.08    );
    AddHisto("track_IP_3pixl1y_eta05"     ,      "3D IP of tracks (3pix)" ,                             nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_3pixl1y_eta05"   ,     "3D IP error (3pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_3pixl1y_eta05"    ,      "3D IP significance (3pix)",           nTrackbins,-35.,35.  );
    AddHisto("track_len_3pixl1y_eta05"     ,     "decay length (3pix)",                         nTrackbins,0,5.     );
    AddHisto("track_dist_3pixl1y_eta05"    ,     "distance to the jet axis (3pix)",                    nTrackbins,0.,0.08    );
    AddHisto("track_IP_3pixl1n_eta05"     ,      "3D IP of tracks (3pix)" ,                             nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_3pixl1n_eta05"   ,     "3D IP error (3pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_3pixl1n_eta05"    ,      "3D IP significance (3pix)",           nTrackbins,-35.,35.  );
    AddHisto("track_len_3pixl1n_eta05"     ,     "decay length (3pix)",                         nTrackbins,0,5.     );
    AddHisto("track_dist_3pixl1n_eta05"    ,     "distance to the jet axis (3pix)",                    nTrackbins,0.,0.08    );
    AddHisto("track_IP_4pixhit_eta05"     ,      "3D IP of tracks (4pix)" ,                             nTrackbins,-0.1,0.1);
    AddHisto("track_IPerr_4pixhit_eta05"   ,     "3D IP error (4pix)",                                 nTrackbins,0,0.1);
    AddHisto("track_IPs_4pixhit_eta05"    ,      "3D IP significance (4pix)",           nTrackbins,-35.,35.  );
    AddHisto("track_len_4pixhit_eta05"     ,     "decay length (4pix)",                         nTrackbins,0,5.     );
    AddHisto("track_dist_4pixhit_eta05"    ,     "distance to the jet axis (4pix)",                    nTrackbins,0.,0.08    );
    ...
    AddHisto("DeepCSV" ,"DeepCSV",                 nDiscbins,0.,1.  );
    AddHisto("DeepCSVP" ,"DeepCSVP",                 nDiscbins,0.,1.  );
    AddHisto("DeepCSVN" ,"DeepCSVN",                 nDiscbins,0.,1.  );
    AddHisto("DeepFlavour" ,"DeepFlavour",                 nDiscbins,0.,1.  );
    AddHisto("CvsL_deep" ,"CvsL_deep",                 nDiscbins,0.,1.  );
    AddHisto("CvsB_deep" ,"CvsB_deep",                 nDiscbins,0.,1.  );
    AddHisto("CvsL_flav" ,"CvsL_flav",                 nDiscbins,0.,1.  );
    AddHisto("CvsB_flav" ,"CvsB_flav",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepCSVb",   "Jet_DeepCSVb",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepCSVc",   "Jet_DeepCSVc",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepCSVl",   "Jet_DeepCSVl",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepCSVbb",   "Jet_DeepCSVb",                 nDiscbins,0.,1.  );
//    AddHisto("prob_DeepCSVcc",   "Jet_DeepCSVcc",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepFlavourB",   "Jet_DeepFlavourB",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepFlavourBB",   "Jet_DeepFlavourBB",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepFlavourLEPB",   "Jet_DeepFlavourLEPB",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepFlavourC",   "Jet_DeepFlavourC",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepFlavourUDS",   "Jet_DeepFlavourUDS",                 nDiscbins,0.,1.  );
    AddHisto("prob_DeepFlavourG",   "Jet_DeepFlavourG",                 nDiscbins,0.,1.  );
    ...
    AddHisto("pfmuonqual_pt",           "pfmuon p_{T}",            50, 0, 100    );
    AddHisto("pfmuonqual_eta",          "pfmuon #eta",            nMubins, -2.5, 2.5 );
    AddHisto("pfmuonqual_phi",        "pfmuon #phi",              nMubins, -1.*pi,pi);
    AddHisto("pfmuonqual_ip",   "3D IP pfmuon",50, -50., 50.   );
    AddHisto("pfmuonqual_ptrel",      "pT rel. of the muon",       nMubins,0,5        );
    AddHisto("pfmuonqual_ratio",      "ratio of pfmuon", nMubins,0,1       );
    AddHisto("pfmuonqual_ratiorel",   "ratioRel of pfmuon", nMubins,0,0.03       );
    AddHisto("pfmuonqual_deltar",       "#DeltaR(pfmuon,jet)",nMubins,0,0.4);

 
    int nElbins=20; 
    AddHisto("pfelectron_multi",      "number of pfelectron",	   7,-0.5,6.5    );
    AddHisto("pfelectron_pt",		"pfelectron p_{T}",  	   50, 0, 100	 );
    AddHisto("pfelectron_eta",   	"pfelectron #eta",  	           nElbins, -2.5, 2.5 );  
    AddHisto("pfelectron_phi",        "pfelectron #phi",              nElbins, -1.*pi,pi);
    AddHisto("pfelectron_ip",	"3D IP of pfelectron",50, -50., 50.   );
    AddHisto("pfelectron_ptrel",      "pT rel. of the pfelectron",	   nElbins,0,5        );
    AddHisto("pfelectron_ratio",      "ratio of pfelectron", nElbins,0,1       );  
    AddHisto("pfelectron_ratiorel",   "ratioRel of pfelectron", nElbins,0,0.03      );  
    AddHisto("pfelectron_deltar",	"#DeltaR(pfelectron,jet)",nElbins,0,0.5);  

    AddHisto("pfelectron_eta_1",  "pfelectron #eta",                 nElbins, -2.5, 2.5 );
    AddHisto("pfelectron_jeta_1", "jet #eta",                 nElbins, -2.5, 2.5 );
    ...
    AddHisto("pfelectron_ratio_1",    "ratio of pfelectron", nElbins,0,1       );
    AddHisto("pfelectron_ratiorel_1",  "ratioRel of pfelectron", nElbins,0,0.03      );
    AddHisto("pfelectron_deltar_1", "#DeltaR(pfelectron,jet)",nElbins,0,0.5);
    AddHisto("pfelectron_eta_2",  "pfelectron #eta",                 nElbins, -2.5, 2.5 );
    AddHisto("pfelectron_jeta_2", "jet #eta",                 nElbins, -2.5, 2.5 );
    ...
    AddHisto("pfelectron_ratio_2",    "ratio of pfelectron", nElbins,0,1       );
    AddHisto("pfelectron_ratiorel_2",  "ratioRel of pfelectron", nElbins,0,0.03      );
    AddHisto("pfelectron_deltar_2", "#DeltaR(pfelectron,jet)",nElbins,0,0.5);
    ...
    AddHisto("df_charged_Sip3dVal","df_charged_Sip3dVal",nTrackbins,-0.1,0.1);
    AddHisto("df_charged_Sip3dSig","df_charged_Sip3dSig",nTrackbins,-35.,35.);
    AddHisto("df_charged_quality","df_charged_quality",40,-10.,10.);
    AddHisto("df_charged_chi2","df_charged_chi2",40,0.,20.);
    AddHisto("df_neutral_drminsv","df_neutral_drminsv",50,-0.5,0.5);
    AddHisto("df_neutral_hadFrac","df_neutral_hadFrac",3,-1.5,1.5);
    AddHisto("df_neutral_ptrel","df_neutral_ptrel",30,-1.5,0.);
    AddHisto("df_sv_d3d","df_sv_d3d", 80, 0.,80.);
    AddHisto("df_sv_d3dsig","df_sv_d3dsig", 80, 0.,800.);
    AddHisto("df_sv_normchi2","df_sv_normchi2",100,-1000.,1000.);
    ...
    if (produceJetProbaTree) {
    AddHisto2D("trackIPs_DeepCSV"    ,    "3D IP significance of all tracks",	    nDiscbins,0.,1.,    nTrackbins,-35.,35.  );
    AddHisto2D("trackIPs_CSVv2"    ,      "3D IP significance of all tracks",	    nDiscbins,0.,1.,    nTrackbins,-35.,35.  );
    AddHisto2D("trackIPs_CvsLdeep"    ,   "3D IP significance of all tracks",	    nDiscbins,0.,1.,    nTrackbins,-35.,35.  );
    AddHisto2D("trackIPs_CvsBdeep"    ,       "3D IP significance of all tracks",	    nDiscbins,0.,1.,    nTrackbins,-35.,35.  );
    AddHisto2D("trackIPs_CvsLold"    ,   "3D IP significance of all tracks",	    nDiscbins,0.,1.,    nTrackbins,-35.,35.  );
    AddHisto2D("trackIPs_CvsBold"    ,       "3D IP significance of all tracks",	    nDiscbins,0.,1.,    nTrackbins,-35.,35.  );
    }
    if (produceSVinfo) {
    AddHisto2D("svFlight3DSig_DeepCSV" ,  "flight distance significance 3D",	    nDiscbins,0.,1.,    nSVbins,0.,80.    );
    AddHisto2D("svFlight3DSig_CSVv2" ,    "flight distance significance 3D",	    nDiscbins,0.,1.,    nSVbins,0.,80.    );
    AddHisto2D("svFlight3DSig_CvsLdeep" ,     "flight distance significance 3D",	    nDiscbins,0.,1.,    nSVbins,0.,80.    );
    AddHisto2D("svFlight3DSig_CvsBdeep" ,     "flight distance significance 3D",	    nDiscbins,0.,1.,    nSVbins,0.,80.    );
    AddHisto2D("svFlight3DSig_CvsLold" ,     "flight distance significance 3D",	    nDiscbins,0.,1.,    nSVbins,0.,80.    );
    AddHisto2D("svFlight3DSig_CvsBold" ,     "flight distance significance 3D",	    nDiscbins,0.,1.,    nSVbins,0.,80.    );
    }
    ...
    int counteur_pfjet40=0;
    int counteur_pfjet60=0;
    int counteur_pfjet80=0;
    int counteur_pfjet140=0;
    int counteur_pfjet200=0;
    int counteur_pfjet260=0;
    int counteur_pfjet320=0;
    int counteur_pfjet400=0;
    int counteur_pfjet450=0;
    int counteur_pfjet500=0;
    int counteur_pfjet550=0;

    int counteur_dijet20=0;
    int counteur_dijet40=0;
    int counteur_dijet70=0;
    int counteur_dijet110=0;
    int counteur_dijet170=0;
    int counteur_jet300=0;
...
//        if (isData && Run!=runtoselect) continue;
...
/*
        ...
        if (sqrtstev<0) ww=1.;
*/
//        if (!isData && trigname=="jet") ww=1000000./906500.; // for QCD off only
//        if (!isData && trigname=="ttbar") ww=100000./99900.; // for ttbar pu off only
        ...
        //-----------------------------------
        Nevent++;
        if(Nevent%2500000 ==0 && Nevent!=0) cout << " number of processed events is " << Nevent <<  " = " << (100.*Nevent)/(1.*nentries) << "%" <<endl;
        int njet_mc    =0; 
        int njet_data  =0; 
        ...
        bool isTrigOK = passTrigger2017(trigname, trigger);
        if (trigname=="ttbar") isTrigOK=true;



        if (passTrigger2017("jet",40)) counteur_pfjet40++;
        if (passTrigger2017("jet",60)) counteur_pfjet60++;
        if (passTrigger2017("jet",80)) counteur_pfjet80++;
        if (passTrigger2017("jet",140)) counteur_pfjet140++;
        if (passTrigger2017("jet",200)) counteur_pfjet200++;
        if (passTrigger2017("jet",260)) counteur_pfjet260++;
        if (passTrigger2017("jet",320)) counteur_pfjet320++;
        if (passTrigger2017("jet",400)) counteur_pfjet400++;
        if (passTrigger2017("jet",450)) counteur_pfjet450++;
        if (passTrigger2017("jet",500)) counteur_pfjet500++;
        if (passTrigger2017("jet",550)) counteur_pfjet550++;

/*
        if (passTrigger2017("btag",20)) counteur_dijet20++;
        if (passTrigger2017("btag",40)) counteur_dijet40++;
        if (passTrigger2017("btag",40)) counteur_dijet40++;
        if (passTrigger2017("btag",70)) counteur_dijet70++;
        if (passTrigger2017("btag",110)) counteur_dijet110++;
        if (passTrigger2017("btag",170)) counteur_dijet170++;
        if (passTrigger2017("btag",300)) counteur_jet300++;
*/
        ...
        //Apply the PS 
        if (setprescale && isData) {
          ww=GetPS(Run,LumiBlock);
        }
        ...
        if(isData){
            ...
            nPV_data       ->Fill(nPV,ww);
            nPV_data_unw       ->Fill(nPV);
        }
        else{
        ...
        for (int ijet = 0; ijet < nJet; ijet++) {
            ...
            int   flav     = Jet_flavourCleaned[ijet];
...
*/
            ...
            float negcsvivf= Jet_CombIVF_N[ijet];
            ...
            float deepcsv = Jet_DeepCSVBDisc[ijet];
            float deepflav = Jet_DeepFlavourBDisc[ijet];
            float cvsl_deep = Jet_DeepCSVCvsLDisc[ijet];
            float cvsb_deep = Jet_DeepCSVCvsBDisc[ijet];
            float cvsl_flav = Jet_DeepFlavourCvsLDisc[ijet];
            float cvsb_flav = Jet_DeepFlavourCvsBDisc[ijet];
           ...
           if ( nsplit > 1 ) isGluonSplit = true; // then you can fill or not the discriminator plots with isGluonSplit true
            ...
            if (csvivf>0.5426 ) {
                ...
                FillHisto_floatFromMap("jet_eta_csvv2_l",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_csvv2_l",                 flav, isGluonSplit ,phijet    ,ww);
                if(isData) njet_csvLoose_data++;
                else njet_csvLoose_mc++;
            }
            if (csvivf>0.8484 ) {
                FillHisto_floatFromMap("jet_pt_csvv2_m",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_csvv2_m",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_csvv2_m",                 flav, isGluonSplit ,phijet    ,ww);
                if(isData) njet_csvMedium_data++;
                else njet_csvMedium_mc++;
            }
            if (csvivf>0.9535 ) {
                FillHisto_floatFromMap("jet_pt_csvv2_t",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_csvv2_t",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_csvv2_t",                 flav, isGluonSplit ,phijet    ,ww);
                if(isData) njet_csvTight_data++;
                else njet_csvTight_mc++;
            }
            if (negcsvivf>0.5426) {
                FillHisto_floatFromMap("jet_phi_negcsvv2_l",                 flav, isGluonSplit ,phijet    ,ww);
            }
            if (negcsvivf>0.8484) {
                FillHisto_floatFromMap("jet_phi_negcsvv2_m",                 flav, isGluonSplit ,phijet    ,ww);
            }
            if (negcsvivf>0.9535) {
                FillHisto_floatFromMap("jet_phi_negcsvv2_t",                 flav, isGluonSplit ,phijet    ,ww);
            }

            // WP of 2018
            if (deepcsv>0.1241 ) {
                FillHisto_floatFromMap("jet_pt_deepcsv_l",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_deepcsv_l",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_deepcsv_l",                 flav, isGluonSplit ,phijet    ,ww);
            }
            if (deepcsv> 0.4184 ) {
                FillHisto_floatFromMap("jet_pt_deepcsv_m",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_deepcsv_m",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_deepcsv_m",                 flav, isGluonSplit ,phijet    ,ww);
            }
            if (deepcsv>0.7527 ) {
                FillHisto_floatFromMap("jet_pt_deepcsv_t",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_deepcsv_t",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_deepcsv_t",                 flav, isGluonSplit ,phijet    ,ww);
            }
            // WP of 2018
            if (deepflav>0.0494) {
                FillHisto_floatFromMap("jet_pt_deepfl_l",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_deepfl_l",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_deepfl_l",                 flav, isGluonSplit ,phijet    ,ww);
            }
            if (deepflav>0.2770) {
                FillHisto_floatFromMap("jet_pt_deepfl_m",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_deepfl_m",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_deepfl_m",                 flav, isGluonSplit ,phijet    ,ww);
            }
            if (deepflav>0.7264) {
                FillHisto_floatFromMap("jet_pt_deepfl_t",                 flav, isGluonSplit ,ptjet    ,ww);
                FillHisto_floatFromMap("jet_eta_deepfl_t",                 flav, isGluonSplit ,etajet    ,ww);
                FillHisto_floatFromMap("jet_phi_deepfl_t",                 flav, isGluonSplit ,phijet    ,ww);
            }

            FillHisto_floatFromMap("DeepCSV",   flav, isGluonSplit, deepcsv         ,   ww);
            FillHisto_floatFromMap("DeepCSVP",   flav, isGluonSplit, Jet_DeepCSVBDiscP[ijet]         ,   ww);
            FillHisto_floatFromMap("DeepCSVN",   flav, isGluonSplit, Jet_DeepCSVBDiscN[ijet]         ,   ww);
            FillHisto_floatFromMap("DeepFlavour",   flav, isGluonSplit, deepflav         ,   ww);
            FillHisto_floatFromMap("CvsL_deep",   flav, isGluonSplit, cvsl_deep         ,   ww);
            FillHisto_floatFromMap("CvsB_deep",   flav, isGluonSplit, cvsb_deep         ,   ww);
            FillHisto_floatFromMap("CvsL_flav",   flav, isGluonSplit, cvsl_flav         ,   ww);
            FillHisto_floatFromMap("CvsB_flav",   flav, isGluonSplit, cvsb_flav         ,   ww);


            FillHisto_floatFromMap("prob_DeepCSVb",   flav, isGluonSplit, Jet_DeepCSVb[ijet]         ,   ww);
            FillHisto_floatFromMap("prob_DeepCSVc",   flav, isGluonSplit, Jet_DeepCSVc[ijet]         ,   ww);
            FillHisto_floatFromMap("prob_DeepCSVl",   flav, isGluonSplit, Jet_DeepCSVl[ijet]         ,   ww);
            FillHisto_floatFromMap("prob_DeepCSVbb",   flav, isGluonSplit, Jet_DeepCSVbb[ijet]       ,   ww);
//            FillHisto_floatFromMap("prob_DeepCSVcc",   flav, isGluonSplit, Jet_DeepCSVcc[ijet]       ,   ww);
            FillHisto_floatFromMap("prob_DeepFlavourB",   flav, isGluonSplit, Jet_DeepFlavourB[ijet]       ,   ww);
            FillHisto_floatFromMap("prob_DeepFlavourBB",   flav, isGluonSplit, Jet_DeepFlavourBB[ijet]       ,   ww);
            FillHisto_floatFromMap("prob_DeepFlavourLEPB",   flav, isGluonSplit, Jet_DeepFlavourLEPB[ijet]       ,   ww);
            FillHisto_floatFromMap("prob_DeepFlavourC",   flav, isGluonSplit, Jet_DeepFlavourC[ijet]       ,   ww);
            FillHisto_floatFromMap("prob_DeepFlavourUDS",   flav, isGluonSplit, Jet_DeepFlavourUDS[ijet]       ,   ww);
            FillHisto_floatFromMap("prob_DeepFlavourG",   flav, isGluonSplit, Jet_DeepFlavourG[ijet]       ,   ww);
            ...
            if ( produceJetProbaTree ) {
                ...
                for (int itrk=Jet_nFirstTrack[ijet]; itrk<Jet_nLastTrack[ijet] ; itrk++){
                    ...
                    FillHisto2D_float_floatFromMap("track_HPix_vs_phi"     ,flav,isGluonSplit ,Track_nHitPixel[itrk], Track_phi[itrk],ww);
                    FillHisto2D_float_floatFromMap("track_eta_vs_phi"     ,flav,isGluonSplit ,Track_eta[itrk], Track_phi[itrk],ww);
                    if (Track_nHitPixel[itrk]==3) {
                        FillHisto2D_float_floatFromMap("track_eta_vs_phi_3pixhit"     ,flav,isGluonSplit ,Track_eta[itrk], Track_phi[itrk],ww);
                        FillHisto_floatFromMap("track_IPerr_3pixhit",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_3pixhit",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_3pixhit",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_3pixhit",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_3pixhit",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                        if (Track_isHitL1[itrk]==1) {
                        FillHisto2D_float_floatFromMap("track_eta_vs_phi_3pixl1y"     ,flav,isGluonSplit ,Track_eta[itrk], Track_phi[itrk],ww);
                        FillHisto_floatFromMap("track_IPerr_3pixl1y",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_3pixl1y",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_3pixl1y",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_3pixl1y",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_3pixl1y",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                        }
                        else {
                        FillHisto2D_float_floatFromMap("track_eta_vs_phi_3pixl1n"     ,flav,isGluonSplit ,Track_eta[itrk], Track_phi[itrk],ww);
                        FillHisto_floatFromMap("track_IPerr_3pixl1n",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_3pixl1n",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_3pixl1n",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_3pixl1n",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_3pixl1n",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                        }
                    }
                    else if (Track_nHitPixel[itrk]==4) {
                        FillHisto2D_float_floatFromMap("track_eta_vs_phi_4pixhit"     ,flav,isGluonSplit ,Track_eta[itrk], Track_phi[itrk],ww);
                        FillHisto_floatFromMap("track_IPerr_4pixhit",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_4pixhit",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_4pixhit",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_4pixhit",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_4pixhit",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                    }
                    if (abs(Track_eta[itrk])<0.5) {
                     if (Track_nHitPixel[itrk]==3) {
                        FillHisto_floatFromMap("track_IPerr_3pixhit_eta05",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_3pixhit_eta05",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_3pixhit_eta05",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_3pixhit_eta05",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_3pixhit_eta05",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                        if (Track_isHitL1[itrk]==1) {
                        FillHisto_floatFromMap("track_IPerr_3pixl1y_eta05",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_3pixl1y_eta05",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_3pixl1y_eta05",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_3pixl1y_eta05",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_3pixl1y_eta05",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                        }
                        else {
                        FillHisto_floatFromMap("track_IPerr_3pixl1n_eta05",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_3pixl1n_eta05",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_3pixl1n_eta05",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_3pixl1n_eta05",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_3pixl1n_eta05",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                        }
                     }
                     else if (Track_nHitPixel[itrk]==4) {
                        FillHisto_floatFromMap("track_IPerr_4pixhit_eta05",   flav, isGluonSplit, Track_IPerr[itrk]    ,ww);
                        FillHisto_floatFromMap("track_IP_4pixhit_eta05",      flav, isGluonSplit ,Track_IP[itrk]       ,ww);
                        FillHisto_floatFromMap("track_IPs_4pixhit_eta05",     flav, isGluonSplit ,Track_IPsig[itrk]    ,ww);
                        FillHisto_floatFromMap("track_len_4pixhit_eta05",     flav, isGluonSplit ,Track_length[itrk]  , ww);
                        FillHisto_floatFromMap("track_dist_4pixhit_eta05",    flav, isGluonSplit ,fabs(Track_dist[itrk])    , ww);
                     }
                    }
                        ...
                        if (sig>sig1_ip) {
                           ...
                           FillHisto2D_float_floatFromMap("trackIPs_DeepCSV"    ,  flav,isGluonSplit , deepcsv, sig, ww);
                           FillHisto2D_float_floatFromMap("trackIPs_CSVv2"    ,    flav,isGluonSplit , csvivf,sig, ww);
                           FillHisto2D_float_floatFromMap("trackIPs_CvsLdeep"    , flav,isGluonSplit , cvsl_deep,sig, ww);
                           FillHisto2D_float_floatFromMap("trackIPs_CvsBdeep"    ,   flav,isGluonSplit ,cvsb_deep,sig, ww);
                           FillHisto2D_float_floatFromMap("trackIPs_CvsLold"    ,   flav,isGluonSplit ,ctagCvsL,sig, ww);
                           FillHisto2D_float_floatFromMap("trackIPs_CvsBold"    ,   flav,isGluonSplit ,ctagCvsB,sig, ww);
            ...
            if (n_sv>0){  
              ...
              if (produceSVinfo) {
                ...
                FillHisto2D_float_floatFromMap("sv_mass_vs_flightDist3D"     ,flav,isGluonSplit ,flightSig_sv,mass_sv,ww);	
                ...
                FillHisto2D_float_floatFromMap("svFlight3DSig_DeepCSV" , flav,isGluonSplit , deepcsv,flightSig_sv ,ww); 
                FillHisto2D_float_floatFromMap("svFlight3DSig_CSVv2" ,  flav,isGluonSplit ,csvivf,flightSig_sv ,ww); 
                FillHisto2D_float_floatFromMap("svFlight3DSig_CvsLdeep" , flav,isGluonSplit ,cvsl_deep,flightSig_sv ,ww); 
                FillHisto2D_float_floatFromMap("svFlight3DSig_CvsBdeep" , flav,isGluonSplit ,cvsb_deep,flightSig_sv ,ww); 
                FillHisto2D_float_floatFromMap("svFlight3DSig_CvsLold" , flav,isGluonSplit ,ctagCvsL,flightSig_sv ,ww); 
                FillHisto2D_float_floatFromMap("svFlight3DSig_CvsBold" , flav,isGluonSplit ,ctagCvsB,flightSig_sv ,ww); 
            ...
            if (indpfmu>-1) {
                    ...
                    if (PFMuon_GoodQuality[indpfmu]>1) {
                    FillHisto_floatFromMap("pfmuonqual_pt",          flav, isGluonSplit, PFMuon_pt[indpfmu],                 ww);
                    FillHisto_floatFromMap("pfmuonqual_eta",     flav, isGluonSplit, PFMuon_eta[indpfmu],                ww);
                    FillHisto_floatFromMap("pfmuonqual_phi",         flav, isGluonSplit, PFMuon_phi[indpfmu],                ww);
                    FillHisto_floatFromMap("pfmuonqual_ip",              flav, isGluonSplit, PFMuon_IP[indpfmu],              ww);
                    FillHisto_floatFromMap("pfmuonqual_ptrel",       flav, isGluonSplit, PFMuon_ptrel[indpfmu],              ww);
                    FillHisto_floatFromMap("pfmuonqual_ratio",       flav, isGluonSplit, PFMuon_ratio[indpfmu],              ww);
                    FillHisto_floatFromMap("pfmuonqual_ratiorel",    flav, isGluonSplit, PFMuon_ratioRel[indpfmu],           ww);
                    FillHisto_floatFromMap("pfmuonqual_deltar",  flav, isGluonSplit, PFMuon_deltaR[indpfmu],             ww);
                    FillHisto2D_float_floatFromMap("pfmuonqual_pt_vs_eta", flav, isGluonSplit,PFMuon_eta[indpfmu],PFMuon_pt[indpfmu],ww);
                    FillHisto2D_float_floatFromMap("pfmuonqual_eta_vs_run", flav, isGluonSplit,Run,fabs(PFMuon_eta[indpfmu]),ww);
                    }
            ...
            FillHisto_floatFromMap("df_charged_Sip3dVal",flav, isGluonSplit, DeepFlavourInput_charged_Sip3dVal[ijet],ww);
            FillHisto_floatFromMap("df_charged_Sip3dSig",flav, isGluonSplit, DeepFlavourInput_charged_Sip3dSig[ijet],ww);
            FillHisto_floatFromMap("df_charged_quality",flav, isGluonSplit, DeepFlavourInput_charged_quality[ijet],ww);
            FillHisto_floatFromMap("df_charged_chi2",flav, isGluonSplit, DeepFlavourInput_charged_chi2[ijet],ww);
            FillHisto_floatFromMap("df_neutral_drminsv",flav, isGluonSplit, DeepFlavourInput_neutral_drminsv[ijet],ww);
            FillHisto_floatFromMap("df_neutral_hadFrac",flav, isGluonSplit, DeepFlavourInput_neutral_hadFrac[ijet],ww);
            FillHisto_floatFromMap("df_neutral_ptrel",flav, isGluonSplit, DeepFlavourInput_neutral_ptrel[ijet],ww);
            FillHisto_floatFromMap("df_sv_d3d",flav, isGluonSplit, DeepFlavourInput_sv_d3d[ijet],ww);
            FillHisto_floatFromMap("df_sv_d3dsig",flav, isGluonSplit, DeepFlavourInput_sv_d3dsig[ijet],ww);
            FillHisto_floatFromMap("df_sv_normchi2",flav, isGluonSplit, DeepFlavourInput_sv_normchi2[ijet],ww);
...
//    cout << "Trigger counter for run " << runtoselect <<  endl;
    cout << "Trigger counter " <<  endl;
    cout << "PFJET 40 : " << counteur_pfjet40 << " , 60: " << counteur_pfjet60 << " , 80: " << counteur_pfjet80 ;
    cout << " , 140 : " << counteur_pfjet140 << " , 200: " << counteur_pfjet200 << " , 260: " << counteur_pfjet260;
    cout << " , 320 : " << counteur_pfjet320 << " , 400: " << counteur_pfjet400 << " , 450: " << counteur_pfjet450;
    cout << " , 500 : " << counteur_pfjet500 << " , 550: " << counteur_pfjet550 << endl;
/*
    cout << "BTAGMU 20 : " << counteur_dijet20 << " , 40: " << counteur_dijet40 << " , 70: " << counteur_dijet70;
    cout << " , 110 : " << counteur_dijet110 << " , 170: " << counteur_dijet170<< " , 300: " << counteur_jet300 << endl;
*/
...
//--------------------------------------------------------------------------------------------------------------------//
bool CommPlotProducer::passTrigger2016(TString trigger, int pttrig) {
    ...
    bool Jet400 = false, Jet450 = false, Jet500 = false, Jet550=false;
    bool BJet20  = false, BJet40  = false, BJet70  = false, BJet110 = false, BJet170 = false, BJet300 = false; 
    ...
    if ( trigger=="jet") {
        ...
        triggerIdx = 10;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) Jet550 = true;
        ...
        if ( pttrig == 550 && Jet550 ) passTrig=true;
        ...
        for (int ijet=0; ijet<nJet ; ijet++) {
            ...
            else if ( pttrig == 550 && ptjet > 575. && etajet < 2.4 ) JetPtCut = true;
        }
        ...
        triggerIdx = 32;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) BJet20  = true;
   
        triggerIdx = 33;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) BJet40  = true;
   
        triggerIdx = 34;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) BJet70  = true;
   
        triggerIdx = 35;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) BJet110 = true;
   
        triggerIdx = 36;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) BJet170 = true;

        triggerIdx = 37;
        bitIdx = int(triggerIdx/32);
        if ( BitTrigger[bitIdx] & ( 1 << (triggerIdx - bitIdx*32) ) ) BJet300 = true;

        if ( pttrig ==  20 && BJet20 )  passTrig=true;
        if ( pttrig ==  40 && BJet40 )  passTrig=true;
        if ( pttrig ==  70 && BJet70 )  passTrig=true;
        if ( pttrig == 110 && BJet110 ) passTrig=true;
        if ( pttrig == 170 && BJet170 ) passTrig=true;
        if ( pttrig == 300 && BJet300 ) passTrig=true;
...
//--------------------------------------------------------------------------------------------------------------------//
bool CommPlotProducer::passTrigger2017(TString trigger, int pttrig) {
    ...
    if ( trigger=="jet") {
        ...
        for (int ijet=0; ijet<nJet ; ijet++) {
            float ptjet = Jet_pt[ijet];
            float etajet = fabs(Jet_eta[ijet]);
            if (      pttrig ==  40 && ptjet >  50. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig ==  60 && ptjet >  70. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig ==  80 && ptjet > 100. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 140 && ptjet > 160. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 200 && ptjet > 220. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 260 && ptjet > 300. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 320 && ptjet > 360. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 400 && ptjet > 425. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 450 && ptjet > 475. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 500 && ptjet > 525. && etajet < 2.5 ) JetPtCut = true;
            else if ( pttrig == 550 && ptjet > 575. && etajet < 2.5 ) JetPtCut = true;
        }
        if (passTrig && JetPtCut) {return true;}
        else {return false;}
    }
        ...
        for (int ijet = 0; ijet < nJet; ijet++) {
            float ptjet = Jet_pt[ijet];
            float etajet = fabs(Jet_eta[ijet]);
            if ( pttrig == 20  &&  ptjet > 30. && etajet < 2.5 )  njtrig++;
            if ( pttrig == 40  &&  ptjet > 50. && etajet < 2.5 )  njtrig++;
            if ( pttrig == 70  &&  ptjet > 80. && etajet < 2.5 )  njtrig++;
            if ( pttrig == 110  &&  ptjet > 120. && etajet < 2.5 ) njtrig++;
            if ( pttrig == 170  &&  ptjet > 190. && etajet < 2.5 ) njtrig++;
            if ( pttrig == 300  &&  ptjet > 330. && etajet < 2.5 ) njtrig++;
        }
        if (passTrig && njtrig>1) {return true;}
        else {return false;}
...
//      
//      LumiBlock
double CommPlotProducer::GetPS(int run, int lb) {
    double w =1.;
    for(int i=0;i<npsVal;i++)
    {
        if( run == _runVal[i] &&
        lb >= _lbVal[i] ) {
            int j = i;
